...
public final class TakeDamageHandler extends AbstractMaplePacketHandler
{
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient client)
    {
                                    ...
                                    {//LOL?
                                        if (Randomizer.nextInt(101) >= loseItem.getChance())
                                        {
                                            if (player.haveItem(loseItem.getId()))
                                            {
                                                ...
                                                MapleInventoryManipulator.removeById(client, type, loseItem.getId(), 1, false, false);
                                                map.spawnItemDrop(client.getPlayer(), client.getPlayer(), new Item(loseItem.getId(), (short) 0, (short) 1), map.calcDropPos(pos, player.getPosition()), true, true);
                                                d++;
                                            }
                                            else
                                            {
        ...
        if (GameConstants.isDojo(map.getId()))
        {
            ...
            client.announce(MaplePacketCreator.getEnergy("energy", player.getDojoEnergy()));
        }
