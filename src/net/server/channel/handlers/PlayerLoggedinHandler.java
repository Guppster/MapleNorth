...
public final class PlayerLoggedinHandler extends AbstractMaplePacketHandler
{
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient client)
    {
        ...
        MapleCharacter player = client.getWorldServer().getPlayerStorage().getCharacterById(cid);
        boolean newcomer = false;
        if (player == null)
        {
            player = MapleCharacter.loadCharFromDB(cid, client, true);
            newcomer = true;
        }
        else
        {
        ...
        for (MapleCharacter character : client.getWorldServer().getPlayerStorage().getAllCharacters())
        {
            if (character.getAccountID() - client.getAccID() == 0)
            {
                if (!character.getName().equalsIgnoreCase(client.getPlayer().getName()))
                {
                    allowLogin = false;
                }
            }
        ...
        {
            for (String charName : client.loadCharacterNames(client.getWorld()))
            {
                for (Channel ch : client.getWorldServer().getChannels())
                {
        ...
        for (CharacterIdChannelPair onlineBuddy : server.getWorld(client.getWorld()).multiBuddyFind(player.getId(), buddyIds))
        {
        ...
        if (player.getGuildId() > 0)
        {
            ...
            else
            {
                ...
                if (allianceId > 0)
                {
                    ...
                    if (newAlliance != null)
                    {
                        client.announce(MaplePacketCreator.updateAllianceInfo(newAlliance, client));
    ...
    private List<Pair<Long, PlayerBuffValueHolder>> getLocalStartTimes(List<PlayerBuffValueHolder> playerBuffValueHolder)
    {
        List<Pair<Long, PlayerBuffValueHolder>> timedBuffs = new ArrayList<>();

        long curtime = System.currentTimeMillis();

        for (PlayerBuffValueHolder buff : playerBuffValueHolder)
        {
            timedBuffs.add(new Pair<>(curtime - buff.getUsedTime(), buff));
        }
